...
export class BranchService {
    private readonly backendUrl = 'http://localhost:5019/api/branches';

    constructor(private http: HttpClient) { }
    getBranches(storeId: number | null, pageNumber: number, pageSize: number): Observable<PaginatedResult<Branch>> {
    ...
    getAllByStoreId(storeId: number, page: number, pageSize: number): Observable<PaginatedResult<Branch>> {
    let params = new HttpParams()
